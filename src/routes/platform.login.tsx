...
/**
 * Standalone entry point for the platform operator — separate from both `/` (the generic
 * per-role login shared by every client) and `/login/$slug` (a specific client's own link).
 * No role selector: this page only ever authenticates the "platform" account, so the choice
 * doesn't exist here. Replaces logging in as platform admin through the generic `/` form.
 */
